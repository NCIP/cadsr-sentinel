...
// $Header: /share/content/gforge/sentinel/sentinel/src/gov/nih/nci/cadsr/sentinel/ui/CRF.java,v 1.3 2007-07-19 15:26:45 hebell Exp $
...
/**
 * A Struts Action class implementation that supports requests made via the DSRAlert.createAlert() method.
 * 
...
public class CRF extends Action
{
        ...
        while (true)
        {
            ...
            if (db.open(request_, ub.getUser()) == 0)
            {
                while (true)
                {
                    ...
                    {
                        // This may be a DE, DEC or VD for the User Reserved CSI.
                        String csi = db.insertAC(list[0], ub.getProxy());
                        if (csi == null || csi.length() == 0)
                            break;
                        
                        // We should use the CSI to create the Alert. Go back to the top of this loop
                        // using the CSI and let's see what happens.
                        list[0] = csi;
                        continue;
                    }
    ...
    private static final Logger _logger = Logger.getLogger(CRF.class.getName());
}
